...
{
    public class SAWHAdapter : SAWAdapter
    {
        /// <summary>
        /// These varibles are required when we connect to SAWH.
        /// </summary>
        public SAWHAppObject sdkObject = new SAWHAppObject();
        public int iServerIP = 0;               //OrganizationID
        public string strDBServerName = "";
        public string strDBName = "";
        public string strRepositoryName = "Default";
        public string strUserName = "";
        public string strPassword = "";
        public int nServerPort = 443;           //if 443, use ssl
        ...
        /// <param name="nServerPort">The Port to indicate if SSL</param>

        public bool Connect(string strServerIP, int nServerPort)
        {
            try
            {
                this.iServerIP = int.Parse(strServerIP);
            }
            catch { }
            this.nServerPort = nServerPort;
            bool bSSL=(nServerPort == 443) ? true : false;
            Enum_EncryptType EncryptType;
            SAWHGlobalMessageSet GlobalMessageSet;
            string strPlanName;
            Boolean Cancelled;
            string ResultDescription;
            if (0 != sdkObject.ConnectToServer(this.iServerIP, false, bSSL,"",0, Enum_ProxyType.Enum_NOPROXY,
                "", 0, "", "", out EncryptType,out GlobalMessageSet,out strPlanName, out Cancelled, out ResultDescription))
        ...
        public bool Login(string strName, string strPassword, string rep)
        {
            ...
            if (0 != sdkObject.Login(strName, strPassword, strRepositoryName, out MustChangePassword,
        ...
        public bool GetDateFromVersion(string path, int version, out System.DateTime date)
        {
            ...
            //if (version < 0)
            //    return GetFileModifiedDate(path, out date);
        ...
        public SAWItemInfo GetFileInfo(string file, int ver)
        {
            ...
            Boolean Cancelled;
            string ResultDescription;
            var set = new SAWHFileCommonInfoSet();
            var info = new SAWHFileCommonInfo();
            info.FileName = file;
            ...
            if (0 == sdkObject.GetFilesCommonInfo(set, out Cancelled, out ResultDescription)
                && set.GetCount() != 0)
            {
                SAWHFileCommonInfo c = new SAWHFileCommonInfo();
        ...
        public int GetLastestVersionNum(string strPath)
        {
            ...
            //string ResultDescription;
            //strPath = SAWCommon.ConvertPath(strPath);
            //if (0 != sdkObject.GetFileLatestVersionNumber(strPath, out VersionLow, out VersionHight, out Cancelled, out ResultDescription))
            //{
            //    if (0 != sdkObject.GetProjectLatestVersionNumber(strPath, out VersionLow, out VersionHight, out Cancelled, out ResultDescription))
            //    {
            //        return -1;
            //    }
            //    else
        ...
        {
            SAWHFileListSet FileListSet = new SAWHFileListSet();
            Boolean Cancelled;
            string ResultDescription;
        ...
        {
            SAWHProjectListSet ProjectListSet = new SAWHProjectListSet();
            Boolean Cancelled;
            string ResultDescription;
            strFolder = SAWCommon.ConvertPath(strFolder);
