...
{
    class SAWSReportHandler : SAWSHandlerBase
    {
                ...
                try
                {
                    ...
                    //    FolderMetaData update = GetMetadataForUpdate(request, (UpdateReportData)data, sourceControlProvider, out targetRevision);
                        ...
                        if(up != null)
                        {
                            ...
                            if (up is FolderMetaData) update = up as FolderMetaData;
                            else
                            {
...
<S:target-revision rev=""0""/>
            ...
            LogItem log = GetLog(localPath, 0, GetSDKObject().GetLastestVersionNum (localPath),
                                                       Recursion.None, 1);
...
'" + serverPath + @"' is not a file
        ...
        private void GetDatedRevReport(TFSSourceControlProvider sourceControlProvider, DatedRevReportData data, TextWriter output, string path)
        {
        //    int targetRevision = sourceControlProvider.GetVersionForDate(data.CreationDate);

            int version = GetSDKObject().GetVersionFromDate(path, data.CreationDate);
        ...
        private void GetLocationsReport(TFSSourceControlProvider sourceControlProvider, GetLocationsReportData getLocationsReport, string path, StreamWriter output)
        {
            ...
            //    ItemMetaData item = sourceControlProvider.GetItemsWithoutProperties(int.Parse(locationRevision), path, Recursion.None);
                if (GetSDKObject().FileExists(path, -1))
                {
      ...
      //      }
        ...
        public FolderMetaData GetChangedItems(string path, int versionFrom, int versionTo, UpdateReportData reportData)
        {
            ...
            var root = (FolderMetaData)QueryItems(path, versionTo, Recursion.Full);
        ...
        private void LogReport(TFSSourceControlProvider sourceControlProvider, LogReportData logreport, string path, TextWriter output)
        {
            string serverPath = "/";
            if (path.IndexOf('/', 9) > -1)
            {
            ...
            LogItem logItem = GetLog(serverPath, Math.Min(start, end), Math.Max(start, end), Recursion.Full, int.Parse(logreport.Limit ?? "1000000"));
            if (start < end)
            {
