...
{
    public class SAWSGetHandler : SAWSHandlerBase 
    {
        protected override void Handle(IHttpContext context, TFSSourceControlProvider sourceControlProvider)
        {
            ...
            if (itemVersion == 0)
            {
                ...
                if (RequestCache.Items[latestVersion] != null) itemVersion = (int)RequestCache.Items[latestVersion];
            }
            ...
            ItemMetaData item = QueryItems(itemPath, itemVersion, Recursion.OneLevel);
            ...
            else if (item.ItemType == ItemType.Folder)
            {
                if (!request.Url.ToString().EndsWith("/"))
                {
                    ...
                    using (StreamWriter writer = new StreamWriter(response.OutputStream))
                    {
                        ...
                        writer.Write("<hr>\n");
                        writer.Write("<address>Apache at " + request.Url.Host + " Port " + request.Url.Port + "</address>\n");
        ...
        private void RenderFile(IHttpContext context, TFSSourceControlProvider sourceControlProvider, ItemMetaData item)
        {
            ...
            byte[] itemData = ReadFile(item.Name, item.ItemRevision);
            if (null != itemData && itemData.Length > 0) // Write throw exception if zero bytes
            {
        ...
        {
            int latestVersion = folder.ItemRevision;
            IHttpResponse response = context.Response;
